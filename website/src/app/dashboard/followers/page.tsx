...
export const metadata: Metadata = {
  title: 'Followers — Episode Ranker',
};
...
/**
 * Plain list of who follows the signed-in user (the click-through target for the follower count on
 * `/dashboard`'s profile header -- see Docs/STATUS.md's dated entry for the `/dashboard` +
 * `/settings` merge that introduced this page). Scoped via `.eq('followee_id', user.id)`, fully
 ...
 * so a private follower still shows up here with their real username/avatar, not hidden or broken
 * (the underlying `follows` row is what actually grants visibility of *that relationship*; the RPC
 * just resolves the safe display columns for whoever's on the other end of it).
 */
export default async function FollowersPage() {
  ...
  const { data } = await supabase.from('follows').select('follower_id').eq('followee_id', user.id);
  const followerIds = ((data ?? []) as { follower_id: string }[]).map((row) => row.follower_id);
  const profiles = await lookupProfileIdentitiesByUserIds(followerIds);
    ...
    <>
      ...
      <div className="flex flex-1 flex-col items-center gap-6 p-8">
        <div className="flex w-full max-w-2xl flex-col gap-4">
          ...
          {profiles.length === 0 ? (
            <p className="text-sm text-black/60 dark:text-white/60">No followers yet.</p>
          ) : (
